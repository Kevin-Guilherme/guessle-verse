/**
 * Atualiza os quests de um build challenge existente com dados frescos do OP.GG (dedup fix).
 * Usage: node _update-build.mjs <challenge_id>
 */
...
function parseArr(s) {
  ...
  return splitTopLevel(s.slice(1, s.lastIndexOf(']')).trim()).map(parseItem).filter(x => x !== null)
}
...
async function fetchBuild(champName, lane, version, ddRunes) {
  ...
  try {
    ...
    let dsl = j.result?.content?.[0]?.text ?? ''
    const pfx = 'LolGetChampionAnalysis(Data('; const dIdx = dsl.indexOf(pfx)
    if (dIdx === -1) return empty
    let pd = 1, ie = dIdx + pfx.length
    for (let i = ie; i < dsl.length; i++) {
      if (dsl[i] === '(') pd++; else if (dsl[i] === ')') { pd--; if (pd === 0) { ie = i; break } }
    }
    ...
    for (let oi = 0; oi < maxOpts && ids.length < 6; oi++)
      for (const slot of extraSlots) { if (ids.length >= 6) break; const opt = slot[oi]; if (opt) addId(opt.ids[0]) }
  ...
  } catch (e) { console.error(`  OP.GG error for ${champName}:`, e.message); return empty }
}

async function main() {
  if (!CHALLENGE_ID) { console.error('Usage: node _update-build.mjs <id>'); process.exit(1) }

  const { data: ch } = await sb.from('daily_challenges').select('id,name,date,extra').eq('id', CHALLENGE_ID).single()
  if (!ch) { console.error(`Challenge ${CHALLENGE_ID} not found`); process.exit(1) }

  const quests = ch.extra?.quests
  if (!quests?.length) { console.error('No quests found in extra'); process.exit(1) }

  console.log(`Updating id=${ch.id} (${ch.date}), champions: ${quests.map(q => q.answer).join(', ')}`)
  ...
  for (const q of quests) {
    console.log(`  Fetching ${q.answer} (${q.build_lane})...`)
    const data = await fetchBuild(q.answer, q.build_lane, version, ddRunes)
    console.log(`    items: ${data.items.length} | skillOrder: ${data.skillOrder.join('')}`)
    newQuests.push({ ...q, build_items: data.items, build_rune_url: data.runeUrl, build_skill_order: data.skillOrder })
  }

  const { error } = await sb.from('daily_challenges')
    .update({ extra: { ...ch.extra, quests: newQuests } })
    .eq('id', CHALLENGE_ID)

  if (error) { console.error('UPDATE failed:', error.message); process.exit(1) }
  console.log(`\nDone. id=${CHALLENGE_ID} updated.`)
  for (const [i, q] of newQuests.entries())
    console.log(`  Quest ${i+1}: ${q.answer} | ${q.build_lane} | ${q.build_items.length} items | ${q.build_skill_order.join('')}`)
}
